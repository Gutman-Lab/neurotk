...
def get_summary_stats_table(data):
    ...
    if len(data):
        ...
        for case in df["npSchema.Emory Case ID"].unique():
            case_df = df[df["npSchema.Emory Case ID"] == case]
...
def update_stacked_barplot(selected_metric, report_store):
    ...
    if len(report_store):
        df = json_normalize(report_store, sep="-")
        ...
        case_order = list(df["npSchema.Emory Case ID"].unique())

        fig = px.bar(
            df,
            x="npSchema.Emory Case ID",
            y=f"results-{selected_metric}",
            color="npSchema.QC Region",
            facet_col="npSchema.Stain",
            category_orders={"npSchema.Stain": sorted(df["npSchema.Stain"].unique())},
            title=f"{selected_metric} by Region and Case",
            labels={
                "npSchema.Emory Case ID": "Case ID",
                "npSchema.QC Region": "Region",
                "npSchema.Stain": "Stain",
            },
        )
...
def return_report_div(ppc_selected_report: str, data: list[dict]):
    ...
    if data:
        if ppc_selected_report == "results":
            return get_results_table(data)
        elif ppc_selected_report == "summary-stats":
            return get_summary_stats_table(data)
